...
test('canonical pending state drives visible priority and portfolio selection regardless of stored label', async ({ page }) => {
  ...
  await row.click()
  await expect(page.locator('.job-detail-constraint')).toContainText('Only one role may be active.')
  await expect(page.locator('.opportunity-detail-decision-reasons')).toContainText('Shared application quota applies')
