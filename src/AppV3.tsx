...
function AppV3() {
  ...
  const todayActions = useMemo(() => selectTodayActions(ranked), [ranked])
...
function TodayView({
  ...
  todayActions: ReturnType<typeof selectTodayActions>
    ...
    <section>
      <header className="page-header compact-header">
        <div>
          ...
          <h1>今天先做什么</h1>
          <p>
            Today 是经过约束后的行动队列：硬截止优先保护，流程复核与复用准备不会无限挤占真正会失效的机会。
          </p>
        </div>
      </header>
      ...
      {todayActions.length === 0 ? (
        <EmptyState
          title="还没有可排序的行动"
          text="到 Import & Settings 导入秋招投递表后，PJSDAS 会在当前浏览器本地生成投递、申请组、流程复核和准备行动。"
        />
      ) : (
        <div className="section-block">
          <div className="section-heading">
            <div>
              <div className="eyebrow">CURATED TODAY QUEUE</div>
              <h2>行动序列</h2>
            </div>
            <span className="muted">{todayActions.length} / {ranked.length} 个活跃行动</span>
          </div>
