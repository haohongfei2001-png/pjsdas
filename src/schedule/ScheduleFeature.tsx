...
function timeLabel(entry: ScheduleEntry, zh: boolean) {
  ...
  if (temporal?.shape === 'availability_window') return (zh ? '开放时间 · ' : 'Window · ') + label
...
export function ScheduleWindowList({ stream, section, opportunities, onOpenOpportunity, className = '' }: {
  ...
  return <div className={'tsui-stream ' + className}>
    {shown.map((entry) => {
      ...
      return <button className="tsui-node-row" type="button" key={entry.id} disabled={!opportunity} onClick={() => { if (entry.opportunityId) onOpenOpportunity(entry.opportunityId) }}>
...
export default function ScheduleFeature({ stream, opportunities, onOpenOpportunity }: {
  ...
  const { lang } = useUiLanguage()
  const zh = lang === 'zh'
  const [section, setSection] = useState<ScheduleSection>('upcoming')
  ...
  return <section className="tsui-schedule-page">
    ...
    <nav className="tsui-schedule-tabs" aria-label={zh ? '日程视图' : 'Schedule views'}>
      {tabs.map(([id, chinese, english]) => <button key={id} type="button" className={section === id ? 'active' : ''} aria-current={section === id ? 'page' : undefined} onClick={() => setSection(id)}>{zh ? chinese : english}<span>{stream.counts[id]}</span></button>)}
    </nav>
