...
function isFixedAction(action: Action) {
  return action.timingMode === 'fixed'
}
...
export function rankAction(
  ...
  if (processTask && isFixedAction(action) && hours !== undefined && hours < 0) {
...
export interface TimePlan {
  ...
  upcomingFixedEvents: RankedAction[]
}
...
export function buildTimePlan(
  ...
  const upcomingFixedEvents = ranked
    .filter((item) => {
  ...
  if (totalMinutes <= budget) {
    const nextHardDeadlines = deadlineWithin48h.filter((item) => !selectedIds.has(item.action.id))
  ...
  const nearDeadlineUnplanned = deadlineWithin48h.filter(
