...
function repairOpportunityIdentities(
  ...
  const operations: ProgressOperation[] = plan.operations.map((operation) => {
    ...
    if (existing.kind === 'match') {
      return {
        ...operation,
        opportunityId: existing.reference.opportunityId,
        company: existing.reference.company,
        role: existing.reference.role,
        confidence: 'high',
      }
    }
...
export function parseProgressUpdate(
  ...
  const identities = repairOpportunityIdentities(
    parseProgressUpdateV4(rawText, currentOpportunities, now),
    currentOpportunities,
    canonicalReferences,
  )
  const shorthand = repairCompanyTestShorthand(identities, currentOpportunities)
