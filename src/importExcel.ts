...
function findOpportunityId(opportunities: Opportunity[], company: string, role: string) {
  ...
  const exact = candidates.find((item) => roleParts.some((part) => item.role.includes(part) || part.includes(item.role)))
  return exact?.id
}
...
export async function parsePJSDASWorkbook(file: File): Promise<ImportBundle> {
  ...
  for (const opportunity of opportunities) {
    if (opportunity.currentStageLabel !== '待投') continue
    if (opportunity.deadline && new Date(opportunity.deadline).getTime() < now.getTime()) continue
  ...
  const processes: ProcessRecord[] = pipeline.flatMap((row, index) => {
    ...
    const nextCheckAt = excelDate(row['下次检查日'], true)
    const silenceRisk = cleanOptional(row['静默风险'])
    ...
    if (nextCheckAt) {
      actions.push({
        ...
        dueAt: nextCheckAt,
        estimatedMinutes: 10,
        leverage: silenceRisk === '需复核' ? 72 : 55,
        delayCost: silenceRisk === '需复核' ? 78 : 45,
        status: 'todo',
        sourceLabel: '在途流程',
        createdAt: importedAt,
        updatedAt: importedAt,
    ...
    return [{
      ...
      stageLabel,
      lastProgressAt: excelDate(row['最后进展日'], true),
      reviewThresholdDays: numberOrUndefined(row['复核阈值(天)']),
      nextCheckAt,
      silenceRisk,
  ...
  const prep: Prep[] = prepRows.flatMap((row) => {
    ...
    if (sourceStatus !== '等待触发') {
      ...
      actions.push({
        ...
        title: `准备｜${title}`,
        prepId: id,
        dueAt: recentNodeAt && new Date(recentNodeAt) > now ? recentNodeAt : undefined,
        estimatedMinutes,
