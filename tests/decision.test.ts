...
function opportunity(overrides: Partial<Opportunity> = {}): Opportunity {
  return {
    id: 'opp', company: 'Test Co', role: 'Test Role', currentStageLabel: '待投', processStage: 'not_applied',
    roleType: 'core', early: false, opportunityValue: 90, fitScore: 75,
    importedAt: new Date(2026, 8, 10, 9, 0).toISOString(), ...overrides,
  }
}

function action(overrides: Partial<Action> = {}): Action {
  return {
    id: 'action', kind: 'apply', title: 'Test action', estimatedMinutes: 30, leverage: 80, delayCost: 70,
    status: 'todo', createdAt: new Date(2026, 8, 10, 9, 0).toISOString(), updatedAt: new Date(2026, 8, 10, 9, 0).toISOString(),
    ...overrides,
  }
}

function process(overrides: Partial<ProcessRecord> = {}): ProcessRecord {
  return { id: 'process', company: 'Test Co', role: 'Test Role', stage: 'screening', stageLabel: '简历筛选', ...overrides }
}
...
describe('pipeline review semantics', () => {
  ...
  it('keeps both current-day and future passive follow-ups out of Today', () => {
    const now = new Date(2026, 8, 10, 11, 36)
    const opportunities = [opportunity({ id: 'opp-1' })]
    const actions = [
      action({ id: 'follow-today', kind: 'follow_up', opportunityId: 'opp-1', dueAt: new Date(2026, 8, 10, 23, 59, 59).toISOString(), estimatedMinutes: 10 }),
      action({ id: 'follow-tomorrow', kind: 'follow_up', opportunityId: 'opp-1', dueAt: new Date(2026, 8, 11, 23, 59, 59).toISOString(), estimatedMinutes: 10 }),
    ]
    expect(rankActions(actions, opportunities, now)).toHaveLength(0)
  })
})

describe('Today queue constraints', () => {
  it('keeps due pipeline review follow-ups out of the curated queue instead of merely capping them', () => {
    const now = new Date(2026, 8, 10, 11, 36)
    const opportunities = [opportunity({ id: 'opp-1' })]
    const actions = Array.from({ length: 4 }, (_, index) => action({
      id: `follow-${index}`, kind: 'follow_up', opportunityId: 'opp-1',
      dueAt: new Date(2026, 8, 9, 23, 59, 59).toISOString(), estimatedMinutes: 10,
    }))
    const selected = selectTodayActions(rankActions(actions, opportunities, now), now, 10)
  ...
  it('never hides same-day hard deadlines when the requested time budget is too small', () => {
    ...
    const actions = [
      action({ id: 'deadline-14', opportunityId: 'urgent-1', dueAt: new Date(2026, 8, 10, 14, 0).toISOString(), estimatedMinutes: 20 }),
      action({ id: 'deadline-eod', opportunityId: 'urgent-2', dueAt: new Date(2026, 8, 10, 23, 59, 59).toISOString(), estimatedMinutes: 90 }),
  ...
  it('prefers a small overrun for the next 48h hard deadline over packing lower-value short tasks', () => {
    ...
    const actions = [
      action({ id: 'today-20', opportunityId: 'today-1', dueAt: new Date(2026, 8, 10, 14, 0).toISOString(), estimatedMinutes: 20 }),
      action({ id: 'today-90', opportunityId: 'today-2', dueAt: new Date(2026, 8, 10, 23, 59, 59).toISOString(), estimatedMinutes: 90 }),
      action({ id: 'tomorrow-90', opportunityId: 'tomorrow', dueAt: new Date(2026, 8, 11, 23, 59, 59).toISOString(), estimatedMinutes: 90 }),
  ...
  it('does not treat a natural-language scheduled assessment date as a recruiter hard deadline', () => {
    ...
    const actions = [
      action({ id: 'dell-apply', opportunityId: 'dell', dueAt: new Date(2026, 8, 11, 23, 59, 59).toISOString(), estimatedMinutes: 90 }),
      action({ id: 'xpeng-assessment', kind: 'manual', opportunityId: 'xpeng', processEventId: 'progress-event:nl:event:xpeng', processStage: 'assessment', dueAt: new Date(2026, 8, 12, 14, 0).toISOString(), timingMode: 'deadline', estimatedMinutes: 45, leverage: 90, delayCost: 92 }),
      action({ id: 'cxmt-scheduled-assessment', kind: 'manual', opportunityId: 'cxmt', processEventId: 'progress-event:nl3:event:cxmt', processStage: 'assessment', dueAt: new Date(2026, 8, 11, 23, 59, 59).toISOString(), timingMode: 'deadline', estimatedMinutes: 45, leverage: 90, delayCost: 92 }),
    ]
