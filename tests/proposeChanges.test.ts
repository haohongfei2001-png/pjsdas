...
import { decodeMcpProposal, encodedProposalFromHash } from '../src/ai/mcpProposal.js'
...
function decodedFrom(result: Awaited<ReturnType<typeof invokeProposeChanges>>) {
  const data = jsonFrom(result)
  const url = new URL(String(data.reviewUrl))
  const encoded = encodedProposalFromHash(url.hash)
  if (!encoded) throw new Error('Expected proposal fragment')
  return decodeMcpProposal(encoded)
}
...
describe('propose_changes', () => {
  it('turns a progress statement into a pending MCP ChangeSet without storing raw source text', async () => {
    ...
    const proposal = decodedFrom(result)
