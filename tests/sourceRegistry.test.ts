...
describe('run-backed ingestion Source Registry', () => {
  ...
  it('persists a changed cadence/SLA in the latest ingestion run and Coverage uses it', () => {
    const snapshot = run(emptySnapshot(), 'monitor:key-changes', '2026-09-13T10:00:00.000Z', {
      version: 1,
      enabled: true,
      label: '关键变化高频检查',
      cadenceMinutes: 12 * 60,
      freshnessSlaMinutes: 18 * 60,
    })
    const registry = effectiveSourceRegistry(snapshot.data.timeline)
    const source = registry.find((item) => item.sourceId === 'monitor:key-changes')
    expect(source).toMatchObject({ policySource: 'run', cadenceMinutes: 720, freshnessSlaMinutes: 1080 })

    const coverage = summarizeCoverage(snapshot.data.timeline, { now: new Date('2026-09-14T05:00:01.000Z') })
  ...
  it('allows a source to disable itself through a durable zero-input run instead of editing Coverage code', () => {
    ...
    const registry = effectiveSourceRegistry(snapshot.data.timeline)
    expect(registry.find((item) => item.sourceId === 'monitor:key-changes')?.enabled).toBe(false)

    const coverage = summarizeCoverage(snapshot.data.timeline, { now: new Date('2026-09-13T10:30:00.000Z') })
  ...
  it('requires an explicit policy for a brand-new trusted source, then adds it to the effective registry', () => {
    expect(() => run(emptySnapshot(), 'monitor:new-source', '2026-09-13T10:00:00.000Z')).toThrow(/sourcePolicy/)
