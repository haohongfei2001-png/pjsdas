...
function changeSet(sourceUrl = 'https://careers.example.com/job-1', discoveredAt = '2026-09-11T10:00:00.000Z', opportunityId = 'job-1'): ChangeSetRecord {
  return {
    ...
    operations: [{
      ...
      summary: '新增发现岗位',
      opportunity: {
        id: opportunityId, company: '甲公司', role: 'AI 产品经理', currentStageLabel: '待投', processStage: 'not_applied', roleType: 'core', early: false,
...
describe('v1.4 discovery inbox', () => {
  ...
  it('keeps similar candidates separate when their exact posting URLs differ', () => {
    const [first] = discoveryInboxItemsFromChangeSet(changeSet('https://jobs.example.com/old', '2026-08-01T00:00:00.000Z', 'job-old'), new Date('2026-08-01T00:05:00.000Z'))
    const [incoming] = discoveryInboxItemsFromChangeSet(changeSet('https://careers.example.com/new', '2026-09-11T10:00:00.000Z', 'job-new'), new Date('2026-09-11T10:05:00.000Z'))
    const merged = mergeDiscoveryInboxItems([first], [incoming], new Date('2026-09-11T10:05:00.000Z'))
    expect(merged).toHaveLength(2)
    expect(merged.map((item) => item.posting?.canonicalSourceUrl).sort()).toEqual([
      'https://careers.example.com/new',
