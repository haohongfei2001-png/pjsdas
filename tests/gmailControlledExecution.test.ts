...
const row = {
  ...
  granted_scopes: [],
  gmail_history_id: 'list-old',
  gmail_reconciliation_requested_at: '2026-09-26T00:30:00Z',
}
...
beforeEach(() => {
  ...
  state.reconcile.mockReset().mockImplementation(async (options) => {
    ...
    return {
      ...
      continuation: {
        version: 1,
        cycleStartedAt: '2026-09-26T00:30:00Z',
        pendingMessageIds: ['pending-1'],
        aggregate: {
          scannedCount: 3,
          recruitingRelevantCount: 1,
          stateCounts: { NO_ACTION: 0, WAITING: 0, ACTION_REQUIRED: 1, COMPLETED: 0, EXPLICITLY_DECLINED: 0, CLOSED: 0, UNRESOLVED: 0 },
          gmailOnlyCount: 0,
          fixedOrHardWithin7DaysCount: 1,
          unavailableMessageCount: 0,
          gmailOpportunityIds: ['opp-1'],
        },
      },
...
describe('opt-in controlled Gmail worker', () => {
  ...
  it('runs reconciliation under the same lease and finalizes without a primary cursor patch', async () => {
    ...
    expect(finish.data.state_patch).toMatchObject({
      cycleComplete: false,
      state: { pendingMessageIds: ['pending-1'] },
    })
