...
describe('public production health contract', () => {
  it('describes the authenticated v1.10 UU-07 integration gateway without exposing demo data or secrets', async () => {
    ...
    expect(body.capabilities.explicitUserCommands).toBe('v1')
    expect(body.capabilities.semanticIntakeContract).toBe('v1')
    expect(body.capabilities.todayBriefReadModel).toBe('v1')
    expect(body.capabilities.todayAgendaReadModel).toBe('v1')
    expect(body.capabilities.opportunityDecisionReadModel).toBe('v1')
    expect(body.capabilities.opportunityConclusionDetail).toBe('v1')
    expect(body.capabilities.latestStartPlanning).toBe('v1')
    expect(body.capabilities.opportunityParticipationState).toBe('v1')
